...
class DoubleEncoder(nn.Module):
    ...
    def forward(self, src):
        ...
        outputs, (hidden, cell) = self.rnn1(embedded)
        outputs, (hidden, cell) = self.rnn2(outputs)
...
class DoubleDecoder(nn.Module):
    ...
    def __init__(self, output_dim=10, emb_dim=64, hid_dim=256, n_layers=2, dropout=0.1):
        ...
        self.rnn1 = nn.LSTM(emb_dim, hid_dim, n_layers, dropout=dropout)

        self.rnn2 = nn.LSTM(hid_dim, 2 * hid_dim, n_layers, dropout=dropout)
    ...
    def forward(self, input, hidden, cell):
        ...
        output, (hidden, cell) = self.rnn1(embedded, (hidden, cell))

        output, (hidden, cell) = self.rnn2(output, (hidden, cell))
